...
async function fetchRepos() {
  ...
  try {
    ...
    // Tech Stack Sorting
    const sort = document.getElementById('techSort')?.value || 'stars';
    if (sort === 'stars') {
      repoResults.sort((a, b) => (b.stargazers_count || 0) - (a.stargazers_count || 0));
    } else if (sort === 'name') {
      repoResults.sort((a, b) => (a.name || "").localeCompare(b.name || ""));
    } else if (sort === 'category') {
      repoResults.sort((a, b) => (a.category || "").localeCompare(b.category || ""));
    }
    ...
    tableBody.innerHTML = `<tr><td colspan="5" style="color:#ef4444; text-align:center; padding: 40px;">âš ï¸ Unable to load technology stack. Please refresh.</td></tr>`;
  }
}
...
function processDemoPayment(planName, amount) {
  ...
  if (!name || !email) {
    showToast('âš ï¸', 'Please fill in your name and email.');
    return;
  }
...
function generateEstimate() {
  ...
  if (!desc) {
    showToast('âš ï¸', 'Please describe your project first.');
    return;
  }
  ...
  setTimeout(() => {
    ...
    output.innerHTML = `
      <div class="estimator-result-header">
        <div class="estimator-result-icon">ðŸ§ </div>
...
function generateSmartEstimate(desc, industry, budget, features) {
  ...
  if (industry) {
    report += `<strong>ðŸ¢ Industry Insight (${industry}):</strong>\n`;
    const insights = {
...
const chatResponses = {
  ...
  'cloud': "We deploy on AWS, Azure, and GCP. We handle migrations, Kubernetes orchestration, and 24/7 monitoring. â˜ï¸",
...
function getBotResponse(msg) {
  ...
  return "Great question! For detailed answers, I'd recommend filling out our contact form or using the AI Estimator above. Our team will get back to you within 2 hours! ðŸš€";
}
...
async function initLaravelProjects() {
  ...
  try {
    ...
    const res = await fetch(`/api/projects?sort=${sort}`);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof VanillaTilt !== 'undefined') {
    VanillaTilt.init(document.querySelectorAll('.hero-code-card, .metric-item, .glass-panel'), {
      max: 5,
      speed: 400,
      glare: true,
      'max-glare': 0.2
