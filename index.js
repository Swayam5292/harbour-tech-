  ...
  try {
    ...
    for (const r of repos) {
      ...
      if (data.message || !data.name) {
        console.warn(`Skipping repo ${cacheKey}: ${data.message || 'No data'}`);
        continue;
      }
      
      // Ensure owner is a string for the data attribute
...
function openDemoCheckout(planName, amount) {
  ...
  overlay.innerHTML = `
    <div class="modal-box" style="max-width:480px; overflow:hidden;">
...
$(document).ready(function () {
  ...
  $("#contactForm").submit(function (e) {
    ...
    saveLead(formData); // Persistent Save
